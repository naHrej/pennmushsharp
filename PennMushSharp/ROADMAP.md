...
| 6. Tooling & Extensibility | Scripting hooks, plugin surface, admin tooling. | Plugin SDK with a Roslyn-based scripting bridge (all in C#), admin CLI/web panel stubs, and a stable plugin hosting surface. |
| 7. Spaceflight Integration | Bring over Aspace simulation as a plugin-hosted module rather than core runtime. | `PennMushSharp.Space` plugin package, tick scheduler, command bindings, telemetry/logging delivered via the SDK. |
...
- ✅ **Social Messaging:** `SAY`, `POSE`, `SEMIPOSE`, `WHISPER`, `PAGE`, `@EMIT`, and `@PEMIT` ride on the new session registry so room residents receive the correct text, and the shared `SpeechService` enforces the `Speech`/`Page` locks plus `HAVEN` rules before delivering output. The parser honors PennMUSH shorthand tokens (`'`, `"`, `:`, `;`, `\`) and routes them to the matching command handlers, keeping telnet UX identical to stock.
...
- ⚠️ **Still Missing:** Attribute authoring (`&attr obj=value`, locks, `$command` processors), MUSHcode helpers (`ufun`, `eval`, `get`), command/function aliases, movement/building commands, parent/zone inheritance (with an opt-in “enhanced inheritance” mode that can exceed legacy limits), event hooks (`@aconnect`, move triggers), a Roslyn-powered plugin SDK (needed before Aspace integration), and richer subsystems like chat/mail (`@conformat`, channel storage). These items block parity and are the next major milestones.
